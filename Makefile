...
SRC_CLI := github.com/D-PlatformOperatingSystem/plugin/cli
APP := build/dplatformos
BUILD_FLAGS = -ldflags "-X github.com/D-PlatformOperatingSystem/dpos/dpos/common/version.GitCommit=`git rev-parse --short=8 HEAD`"
LDFLAGS := -ldflags "-w -s"
...
build: depends
	...
	go build $(BUILD_FLAGS) -v -i -o build/fork-config github.com/D-PlatformOperatingSystem/plugin/cli/fork_config/
...
update: ## version    git tag       ,    commit hash,             master
	@if [ -n "$(version)" ]; then   \
	go get -v github.com/D-PlatformOperatingSystem/dpos@${version}  ; \
	else \
	go get -v github.com/D-PlatformOperatingSystem/dpos@master ;fi
	@go mod tidy
dep:
...
addupstream:
	git remote add upstream https://github.com/D-PlatformOperatingSystem/plugin.git
	git remote -v
