...
function transfer() {
    ...
    hashes=()
    for ((i = 0; i < 10; i++)); do
        hash=$(${1} send coins transfer -a 1 -n test -t 16ERTbYtKKQ64wMthAY9J4La4nAiidG45A -k 4257D8692EF7FE13C68B65D6A52F03933DB2FA5CE8FAF210B5B8B80C721CED01)
        hashes=("${hashes[@]}" "$hash")
    done
    block_wait "${1}" 1
    echo "len: ${#hashes[@]}"
    ...
    #    block_wait "${1}" 2
    tx_wait "${1}" "${hash}"
    before=$(${1} account balance -a 16ERTbYtKKQ64wMthAY9J4La4nAiidG45A -e retrieve | jq -r ".balance")
