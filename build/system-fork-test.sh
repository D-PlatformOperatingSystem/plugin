...
function start() {
    ...
    result=$(${CLI} seed save -p 1314dpos -s "tortoise main civil member grace happy century convince father cage beach hip maid merry rib" | jq ".isok")
    ...
    echo "=========== # unlock wallet ============="
    result=$(${CLI} wallet unlock -p 1314dpos -t 0 | jq ".isok")
    if [ "${result}" = "false" ]; then
        exit 1
    fi
    ...
    ## 2nd mining
    echo "=========== # save seed to wallet ============="
    result=$(${CLI4} seed save -p 1314dpos -s "tortoise main civil member grace happy century convince father cage beach hip maid merry rib" | jq ".isok")
    if [ "${result}" = "false" ]; then
        echo "save seed to wallet error seed, result: ${result}"
        exit 1
    fi
    ...
    echo "=========== # unlock wallet ============="
    result=$(${CLI4} wallet unlock -p 1314dpos -t 0 | jq ".isok")
    if [ "${result}" = "false" ]; then
        exit 1
    fi
