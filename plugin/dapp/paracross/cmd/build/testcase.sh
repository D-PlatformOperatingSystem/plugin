...
function para_set_toml() {
    ...
    sed -i $xsedfix 's/^tokenApprs=.*/tokenApprs=[	"1Bsg9j6gW83sShoee1fZAt9TkUjcrCgA9S",\
	                                                ...
	                                                "12oupcayRT7LvaC4qW4avxsTE7U41cKSio",]/g' "${1}"
    #autonomy
...
function para_set_wallet() {
    ...
    #16ERTbYtKKQ64wMthAY9J4La4nAiidG45A
...
function para_import_wallet() {
    ...
    result=$(${1} seed save -p 1314dpos -s "tortoise main civil member grace happy century convince father cage beach hip maid merry rib" | jq ".isok")
    ...
    echo "=========== # unlock wallet ============="
    result=$(${1} wallet unlock -p 1314dpos -t 0 | jq ".isok")
    if [ "${result}" = "false" ]; then
        exit 1
    fi
...
function para_transfer() {
    ...
    #relay rpc test
    para_transfer2account "${PARA_CLI}" "12oupcayRT7LvaC4qW4avxsTE7U41cKSio"
    ...
    #token precreate
    txhash=$(para_configkey "${PARA_CLI}" "token-blacklist" "DOM")
    echo "txhash=$txhash"
...
function para_cross_transfer_withdraw() {
    ...
    hash=$(${CLI} send para asset_transfer --paraName user.p.para. -a 1.4 -n test -t 12oupcayRT7LvaC4qW4avxsTE7U41cKSio -k 4257D8692EF7FE13C68B65D6A52F03933DB2FA5CE8FAF210B5B8B80C721CED01)
    echo "${hash}"
    query_tx "${PARA_CLI}" "${hash}"

    hash2=$(${CLI} send para asset_withdraw --paraName user.p.para. -a 0.7 -n test -t 12oupcayRT7LvaC4qW4avxsTE7U41cKSio -k 4257D8692EF7FE13C68B65D6A52F03933DB2FA5CE8FAF210B5B8B80C721CED01)

    local times=200
    while true; do
        acc=$(${CLI} account balance -e paracross -a 12oupcayRT7LvaC4qW4avxsTE7U41cKSio | jq -r ".balance")
        acc_para=$(${PARA_CLI} asset balance -a 12oupcayRT7LvaC4qW4avxsTE7U41cKSio --asset_exec paracross --asset_symbol coins.dpos | jq -r ".balance")
        ...
        if [ "${acc}" != "9.3000" ] || [ "${acc_para}" != "0.7000" ]; then
            ...
            if [ $times -le 0 ]; then
                ...
                ${PARA_CLI} asset balance -a 12oupcayRT7LvaC4qW4avxsTE7U41cKSio --asset_exec paracross --asset_symbol coins.dpos
                exit 1
            fi
        else
...
function token_create_on_mainChain() {
    ...
    hash=$(${CLI} send config config_tx -c token-blacklist -o add -v DOM -k 0xc34b5d9d44ac7b754806f761d3d4d2c4fe5214f6b074c19f069c4f5c2a29c8cc)
    echo "${hash}"
    query_tx "${MAIN_CLI}" "${hash}"

    echo "=========== # send coins to token ============="
    hash=$(${CLI} send coins send_exec -a 500 -e token -n send2exec -k 12oupcayRT7LvaC4qW4avxsTE7U41cKSio)
    echo "${hash}"
    query_tx "${MAIN_CLI}" "${hash}"

    echo "=========== # 1.token precreate token DOM ============="
    hash=$(${CLI} send token precreate -f 0.001 -i test -n guodunjifen -a 12oupcayRT7LvaC4qW4avxsTE7U41cKSio -p 0 -s DOM -t 10000 -k 12oupcayRT7LvaC4qW4avxsTE7U41cKSio)
    ...
    if [ "${owner}" != "12oupcayRT7LvaC4qW4avxsTE7U41cKSio" ]; then
        echo "wrong pre create owner"
        exit 1
    fi
    ...
    echo "=========== # 2.token finish ============="
    hash=$(${CLI} send token finish -f 0.001 -a 12oupcayRT7LvaC4qW4avxsTE7U41cKSio -s DOM -k 0xc34b5d9d44ac7b754806f761d3d4d2c4fe5214f6b074c19f069c4f5c2a29c8cc)
    echo "${hash}"
    query_tx "${MAIN_CLI}" "${hash}"
    ...
    ${CLI} token balance -a 12oupcayRT7LvaC4qW4avxsTE7U41cKSio -e token -s DOM
    balance=$(${CLI} token balance -a 12oupcayRT7LvaC4qW4avxsTE7U41cKSio -e token -s DOM | jq -r '.[]|.balance')
    if [ "${balance}" != "10000.0000" ]; then
        echo "wrong para token genesis create, should be 10000.0000"
        exit 1
    fi
...
function para_cross_transfer_withdraw_for_token() {
    ...
    echo "=========== # 1.transfer token:DOM to paracross ============="
    hash=$(${CLI} send token send_exec -a 333 -s DOM -e paracross -k 12oupcayRT7LvaC4qW4avxsTE7U41cKSio)
    echo "${hash}"
    query_tx "${MAIN_CLI}" "${hash}"

    echo "=========== # 2.transfer asset to para chain ============="
    hash=$(${CLI} send para asset_transfer --paraName user.p.para. -s DOM -a 220 -n test -t 12oupcayRT7LvaC4qW4avxsTE7U41cKSio -k 12oupcayRT7LvaC4qW4avxsTE7U41cKSio)
    echo "${hash}"
    query_tx "${MAIN_CLI}" "${hash}"

    echo "=========== # 3.asset_withdraw from parachain ============="
    ${CLI} send para asset_withdraw --paraName user.p.para. -a 111 -s DOM -n test -t 12oupcayRT7LvaC4qW4avxsTE7U41cKSio -k 12oupcayRT7LvaC4qW4avxsTE7U41cKSio

    local times=100
    while true; do
        acc=$(${CLI} asset balance -a 12oupcayRT7LvaC4qW4avxsTE7U41cKSio --asset_symbol DOM --asset_exec token -e paracross | jq -r ".balance")
        acc_para=$(${PARA_CLI} asset balance -a 12oupcayRT7LvaC4qW4avxsTE7U41cKSio --asset_symbol token.dom --asset_exec paracross -e paracross | jq -r ".balance")
...
function privacy_transfer_test() {
    ...
    ${MAIN_CLI} send coins transfer -a 1 -t 15XvcMYK6H1La7ns4yzJhkyurdpXsjjzfQ -k 12oupcayRT7LvaC4qW4avxsTE7U41cKSio
    ${1} send coins transfer -a 10 -t 1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4 -k 16ERTbYtKKQ64wMthAY9J4La4nAiidG45A
    block_wait "${1}" 2
    ...
    echo "#privacy pub2priv, to=16ERTbYtKKQ64wMthAY9J4La4nAiidG45A"
    ${1} send privacy pub2priv -a 9 -p fcbb75f2b96b6d41f301f2d1abc853d697818427819f412f8e4b4e12cacc0814d2c3914b27bea9151b8968ed1732bd241c8788a332b295b731aee8d39a060388 -e coins -s DOM -k 1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4
    ...
    check_privacy_utxo "${1}" 16ERTbYtKKQ64wMthAY9J4La4nAiidG45A token GD 9.0000
    check_privacy_utxo "${1}" 16ERTbYtKKQ64wMthAY9J4La4nAiidG45A coins DOM 9.0000

    echo "#privacy priv2priv, to=1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4"
    ${1} send privacy priv2priv -a 3 -p 5b0ff936ec2d2825a67a270e34d741d96bf6afe5d4b5692de0a1627f635fd0b3d7b14e44d3f8f7526030a7c59de482084161b441a5d66b483d80316e3b91482b -f 16ERTbYtKKQ64wMthAY9J4La4nAiidG45A -e coins -s DOM -k 16ERTbYtKKQ64wMthAY9J4La4nAiidG45A
    ${1} send privacy priv2priv -a 3 -p 5b0ff936ec2d2825a67a270e34d741d96bf6afe5d4b5692de0a1627f635fd0b3d7b14e44d3f8f7526030a7c59de482084161b441a5d66b483d80316e3b91482b -f 16ERTbYtKKQ64wMthAY9J4La4nAiidG45A -e token -s GD -k 16ERTbYtKKQ64wMthAY9J4La4nAiidG45A
    check_privacy_utxo "${1}" 1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4 token GD 3.0000
    check_privacy_utxo "${1}" 1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4 coins DOM 3.0000

    echo "#privacy priv2pub, to=1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4"
    ${1} send privacy priv2pub -a 6 -t 1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4 -f 16ERTbYtKKQ64wMthAY9J4La4nAiidG45A -e coins -s DOM -k 16ERTbYtKKQ64wMthAY9J4La4nAiidG45A
    ${1} send privacy priv2pub -a 6 -t 1KSBd17H7ZK8iT37aJztFB22XGwsPTdwE4 -f 16ERTbYtKKQ64wMthAY9J4La4nAiidG45A -e token -s GD -k 16ERTbYtKKQ64wMthAY9J4La4nAiidG45A
    check_privacy_utxo "${1}" 16ERTbYtKKQ64wMthAY9J4La4nAiidG45A token GD 0.0000
    check_privacy_utxo "${1}" 16ERTbYtKKQ64wMthAY9J4La4nAiidG45A coins DOM 0.0000
}
