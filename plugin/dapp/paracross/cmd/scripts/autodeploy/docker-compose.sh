...
DplatformOS_CLI="dplatformos-cli"
...
function para_import_wallet() {
    ...
    ./$DplatformOS_CLI --rpc_laddr "http://localhost:$port" seed save -p 1314dpos -s "tortoise main civil member grace happy century convince father cage beach hip maid merry rib"

    echo "=========== # unlock wallet ============="
    ./$DplatformOS_CLI --rpc_laddr "http://localhost:$port" wallet unlock -p 1314dpos -t 0

    echo "=========== # import private key ============="
    echo "key: ${key}"
    ./$DplatformOS_CLI --rpc_laddr "http://localhost:$port" account import_key -k "${key}" -l "paraAuthAccount"

    echo "=========== # close auto mining ============="
    ./$DplatformOS_CLI --rpc_laddr "http://localhost:$port" wallet auto_mine -f 0

    echo "=========== # wallet status ============="
    ./$DplatformOS_CLI --rpc_laddr "http://localhost:$port" wallet status
}

function para_unlock_wallet() {
    for ((i = 0; i < ${#authPort[@]}; i++)); do
        echo "=========== # para unlock wallet ${authPort[$i]}============="
        ./$DplatformOS_CLI --rpc_laddr "http://localhost:${authPort[$i]}" wallet unlock -p 1314dpos -t 0
    done
