...
import (
	...
	cmdtypes "github.com/D-PlatformOperatingSystem/dpos/system/dapp/commands/types"
...
func createPub2PrivTxFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringP("symbol", "s", "DOM", "asset symbol, default DOM")
...
func addListPrivacyTxsFlags(cmd *cobra.Command) {
	...
	cmd.Flags().StringP("exec", "e", "coins", "asset executor(coins, token, paracross), default coins")
	cmd.Flags().StringP("token", "", types.DOM, "token name.(DOM supported)")
