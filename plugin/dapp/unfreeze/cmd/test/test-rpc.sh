...
function query_unfreezeID() {
    ...
    local times=10
    while true; do
        req='{"method":"DplatformOS.QueryTransaction","params":[{"hash":"'"$txhash"'"}]}'
...
function init() {
    ...
    dplatformos_ImportPrivkey "${beneficiary_key}" "${beneficiary}" "unfreeze_beneficiary" "${MAIN_HTTP}"
    dplatformos_ImportPrivkey "${owner_key}" "${owner}" "unfreeze_owner" "${MAIN_HTTP}"

    dplatformos_applyCoins "${beneficiary}" 10000000000 "${MAIN_HTTP}"
    dplatformos_applyCoins "${owner}" 10000000000 "${MAIN_HTTP}"
    ...
    if [ "$ispara" == true ]; then
        ...
        dplatformos_applyCoins "${owner}" 10000000000 "${main_ip}"
    fi
    ...
    dplatformos_SendToAddress "$owner" "$exec_addr" 5000000000 "${MAIN_HTTP}"
    dplatformos_SendToAddress "$beneficiary" "$exec_addr" 5000000000 "${MAIN_HTTP}"
    dplatformos_BlockWait 1 "${MAIN_HTTP}"
}
...
function CreateRawUnfreezeCreate() {
    ...
    dplatformos_SignAndSendTx "$RETURN_RESP" "$owner_key" "${MAIN_HTTP}"
    query_unfreezeID
}
...
function rpc_test() {
    dplatformos_RpcTestBegin unfreeze
    MAIN_HTTP="$1"
    echo "main_ip=$MAIN_HTTP"

    init
    run_testcases
    dplatformos_RpcTestRst unfreeze "$CASE_ERR"
}
